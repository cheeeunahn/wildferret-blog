...
쓰지 말라는 쪽의 이유는 순환성(circularity)이다. LLM은 학습한 데이터에 있는 패턴을 조합하는 도구라서 그 바깥의 이야기는 만들지 못한다. 그러니 나오는 건 누군가의 구체적인 경험이 아니라 어디서 본 듯한 일반적인 유저 묘사다. **새로운 사용성 문제나 미처 몰랐던 니즈를 찾는 데는 쓸 수 없다는 뜻이고,** 아직 인터넷에 이야기가 쌓이지 않은 새 제품일수록 이 한계가 크게 걸린다.
